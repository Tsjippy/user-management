...
function changeVisibility(target) {
  target
    .closest("li")
    .querySelectorAll(".ministryposition")
    .forEach((label) => {
...
async function addNewMinistry(target) {
  var response = await FormSubmit.submitForm(
    target,
    "user_management/add_ministry",
  );

  if (response) {
    var ministryName = target
      .closest("form")
      .querySelector('[name="location-name"]').value;
...
document.addEventListener("click", function (event) {
  ...
  if (target.id == "add-ministry-button") {
    ...
    target.closest("li").querySelector(".ministry-option-checkbox").checked =
      false;
...
function onBlur(ev) {
  document
    .querySelectorAll(
      `.ministryposition input[type="text"][name="${ev.target.name}"]`,
    )
    .forEach((input) => {
      // set value
      ...
      // check the ministry
      input.closest("li").querySelector(".ministry-option-checkbox").checked =
        true;
    });
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .querySelectorAll('.ministryposition input[type="text"]')
    .forEach((el) => {
